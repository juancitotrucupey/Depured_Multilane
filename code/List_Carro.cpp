...
void Carro::Calcule_Gap_Adelante(int position_adelante, int velocity_adelante){
  Gap_Position_Ahead=position_adelante-Position-2;
  Gap_Velocity_Ahead=velocity_adelante-Velocity;
  Gap_Ahead=Gap_Position_Ahead+Gap_Velocity_Ahead+1;
  if(Gap_Position_Ahead<0){
    std::cout<<""<<std::endl;
...
bool Carro::Is_Safe_To_Change_Lines_Izq_Ahead(void){
  if(Carro_Izq_Adelante==(*Line).No_Existe_Carril_Vecino){
  ...
  else if(Gap_Position_Izq_Back<0){
    ...
    if(vel+(*Line).Safe_Exceding_Distance[vel]<=(Gap_Position_Izq_Ahead)){
      vel_2=vel;
	...
	if(!(*Carro_Izq_Adelante).Get_Estacionarias()){
	  return true;
		...
		((Gap_Position_Izq_Ahead)>=(*Line).Safe_Braking_Distance[vel_2+1]+vel_2+1)){
	  return true;
	}
	else{
	  false;
...
bool Carro::Is_Safe_To_Change_Lines_Izq_Back(void){
  if(Carro_Izq_Atras==(*Line).No_Existe_Carril_Vecino){
    ...
    if(vel_back+(*Line).Safe_Exceding_Distance[vel_back]<=(Gap_Position_Izq_Back)){
...
bool Carro::Is_Safe_To_Change_Lines_Der_Ahead(void){
  if(Carro_Der_Adelante==(*Line).No_Existe_Carril_Vecino){
  ...
  else if(Gap_Position_Der_Back<0){
    //La posicion de destino esta ocupada	
    return false;
  }
    ...
    if(vel+(*Line).Safe_Exceding_Distance[vel]<=(Gap_Position_Der_Ahead)){
      vel_2=vel;
	...
	if(!(*Carro_Der_Adelante).Get_Estacionarias()){
	  return true;
		...
		((Gap_Position_Der_Ahead)>=(*Line).Safe_Braking_Distance[vel_2+1]+vel_2+1)){
	  return true;
	}
	else{
	  false;
...
bool Carro::Is_Safe_To_Change_Lines_Der_Back(void){
  if(Carro_Der_Atras==(*Line).No_Existe_Carril_Vecino){
    ...
    if(vel_back+(*Line).Safe_Exceding_Distance[vel_back]<=(Gap_Position_Der_Back)){
...
bool Carro::Is_Worth_To_Ask_For_Way_Derecha(void){
  if(Carro_Der_Atras==(*Line).No_Existe_Carril_Vecino){
    ...
    if((-Gap_Position_Der_Back+vel_back>=4)&&(Gap_Position_Der_Back+carro_atras_gap_pos_back>=-2)){
...
bool Carro::Is_Worth_To_Ask_For_Way_Izquierda(void){
  if(Carro_Izq_Atras==(*Line).No_Existe_Carril_Vecino){
    ...
    if((-Gap_Position_Izq_Back+vel_back>=4)&&(Gap_Position_Izq_Back+carro_atras_gap_pos_back>=-2)){
      //El carro ve factible entrar al carril en el siguiente paso de tiempo
      return true;
    }	
    else{
...
void Carro::Decide_Change_Lines_And_Direccionales(int tipo,bool direccionales,Crandom & ran2){
  ...
  if((*Carro_Adelante).Get_Estacionarias()&&direccionales&&
     ((Gap_Position_Ahead)<=(*Line).Safe_Braking_Distance[Velocity+1]+Velocity+1)){
    if(Carro_Der_Atras!=(*Line).No_Existe_Carril_Vecino){
      Direccional_Derecha=true;
    }
    if(Carro_Izq_Atras!=(*Line).No_Existe_Carril_Vecino){
...
bool Carro::Is_Safe_To_Give_Way_Right(int & vel_after_giving_way){
  ...
  if(Ceder_Paso_Derecha){
    if((Gap_Position_Der_Ahead)>(*Line).Safe_Braking_Distance[vel_after_giving_way+1]+vel_after_giving_way+1){
      return false;
    }
    else if((Gap_Position_Der_Ahead)>=(*Line).Safe_Braking_Distance[vel_after_giving_way]+vel_after_giving_way){
...
bool Carro::Is_Safe_To_Give_Way_Left(int & vel_after_giving_way){
  if(Ceder_Paso_Izquierda){
    if((Gap_Position_Izq_Ahead)>(*Line).Safe_Braking_Distance[vel_after_giving_way+1]+vel_after_giving_way+1){
      return false;
    }
    else if((Gap_Position_Izq_Ahead)>=(*Line).Safe_Braking_Distance[vel_after_giving_way]+vel_after_giving_way){
      vel_after_giving_way=vel_after_giving_way;
    }
    else{
      vel_after_giving_way=(*Line).Safe_Minimum_Next_Velocity[vel_after_giving_way];
...
void Carro::Calcular_Nuevo_Estado(int& vel, int& delay, bool& brake, bool& estacionarias, int& tiempo_estacionarias, bool& stoping_safely){
  ...
  if(stoping_safely){
    ...
    if(velocidad>vel){
      if(Gap_Position_Ahead>=(*Line).Safe_Braking_Distance[velocidad]+vel){
	vel=velocidad;
	delay=retardo;
	brake=freno;
      }
    ...
    if(vel>=(Gap_Position_Ahead)){
      vel=std::max(0,Gap_Position_Ahead-1);
      delay=0;
      brake=true;
    }
  }
