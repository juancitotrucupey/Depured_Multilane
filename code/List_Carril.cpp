...
void Carril::Fijar_Carros_Condiciones_De_Frontera(void){
    ...
    punto_salida=Largo+Largo_Salida;
		...
		if(!Carril_1.empty()){
			posicion=std::min((*Primer_Carro).Get_Position()-((*carro_frontera_entrada).Get_Gap_Posicion_Adelante()+1),punto_entrada);
		}
		else{
			posicion=punto_entrada-((*carro_frontera_entrada).Get_Gap_Posicion_Adelante()+1);
		}
		...
		gap_posicion_atras=(*carro_frontera_entrada).Get_Gap_Posicion_Atras();
    ...
    (*Carro_Frontera_Entrada).Set_Pointer_Carril(Line);
		...
		posicion=punto_salida+(*carro_frontera_salida).Get_Gap_Posicion_Atras();
		...
		gap_posicion_adelante=(*carro_frontera_salida).Get_Gap_Posicion_Adelante();
...
void Carril::Interaccion_Con_Carriles_Frontera(void){
  	if(!Fronteras_Periodicas){
        ...
        if(!Carril_1.empty()){
            posicion=std::min((*Primer_Carro).Get_Position()-((*carro_sencillo).Get_Gap_Posicion_Adelante()+1),punto_entrada);
        }
        else{
            posicion=punto_entrada-((*carro_sencillo).Get_Gap_Posicion_Adelante()+1);
        }
        while(carro_sencillo!=(*Carril_Frontera_Entrada).Primer_Carro){
            if(posicion>=punto_entrada-Solapamiento_Con_Frontera_Entrada){
                ...
                posicion-=((*carro_sencillo).Get_Gap_Posicion_Adelante()+1);
            }
            else{
                break;
            }
